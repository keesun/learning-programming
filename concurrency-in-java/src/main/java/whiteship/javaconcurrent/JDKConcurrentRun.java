...
public class JDKConcurrentRun extends AbstractRun {
    ...
    private synchronized void updateValues(final String ticker, final double value, final double price) {
        netAssetValue += value;
        if (price < lowPrice) {
            lowPrice = price;
    ...
    protected Map<String, Object> computeFinancials() {
        ...
        try {
            for (int i = 0; i < Stocks.tickers.length; i++) {
                ...
                service.execute(new Runnable() {
                    public void run() {
                        ...
                        updateValues(Stocks.tickers[index], Stocks.shares[index] * price, price);
                        latch.countDown();
                    }
                });
            }
            ...
            result.put("HIGHTICKER", highPricedTicker); result.put("HIGHPRICE", highPrice);
            result.put("LOWTICKER", lowPricedTicker); result.put("LOWPRICE", lowPrice);
